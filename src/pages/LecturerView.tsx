...
interface LecturerSlideProps {
  slide: Slide;
  dayColor: string;
  slideNumber: number;
}

const LecturerSlide: React.FC<LecturerSlideProps> = ({
  slide,
  dayColor,
  slideNumber
}) => {
  ...
  return (
    <div className={`lec-slide ${isTitle ? "lec-slide--title" : ""}`}>
      ...
      {slide.title && (
        <h4 className="lec-slide-title" style={{ "--day-color": dayColor }}>
          {slide.title}
        </h4>
      )}
      {slide.subtitle && <p className="lec-slide-subtitle">{slide.subtitle}</p>}
...
const LecturerView: React.FC<LecturerViewProps> = ({ courseData, onBack }) => {
                ...
                <div key={pres.id} className="lec-lesson">
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setExercises({ presId: pres.id, presTitle: pres.title })
                      }
                    >
                      <GitHubIcon />
                      תרגילים
